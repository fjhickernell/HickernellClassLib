...
& = [M_Y(t/\sqrt{n})]^n
\lim_{n \to \infty}M_{Z_n}(t) = [M(0)]^\infty = 1^\infty \quad \class{alert}{\text{(undetermined)}}
\end{align*}
...
- These guaranteed Markov and Cheybshev bounds still depend on knowing or approximating $\sigma^2$ or $\Ex \lvert X - \med(X)\rvert$
...
Define the sample mean $\barX_n := \frac1n\sum_{i=1}^n X_i$.  Then $\barX_n \convp \mu$.
