...
- [Quantile function]{.alert} $Q$ of a random variable $X$ plays the role of the [inverse]{.alert} of the CDF [(left-continuous)]{.alert}
...
- [Variance]{.alert} of $X$ is $\sigma^2 : = \var(X) := \Ex[(X - \mu)^2] \exeq E(X^2) - \mu^2$
...
$$
\varrho(\vx): = \frac{\exp\bigl(-(\vx-\vmu)^\top \mSigma^{-1} (\vx-\vmu) /2\bigr)}{\sqrt{(2 \pi)^{d} \det(\mSigma)}};\; \Ex(\vX) = \vmu, \; \cov(\vX) = \mSigma
\class{alert}{\text{ symmetric, positive definite}}
$$