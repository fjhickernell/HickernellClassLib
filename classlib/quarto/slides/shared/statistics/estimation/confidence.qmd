...
x_lo = mu_hat - 4*se_hat
x_hi = mu_hat + 4*se_hat
x = np.linspace(x_lo, x_hi, 900)

a2 = alpha/2

# Student-t CI endpoints
tcrit = st.t.ppf(1 - a2, df)
x_t_lo = mu_hat - tcrit * se_hat
x_t_hi = mu_hat + tcrit * se_hat

# CLT (z) CI endpoints
zcrit = st.norm.ppf(1 - a2)
x_clt_lo = mu_hat - zcrit * se_hat
x_clt_hi = mu_hat + zcrit * se_hat

fig, ax = plt.subplots()

# Same exact sampling pdf for Xbar (Normal), plot it once (black)
ax.plot(x, pdf_xbar(x), linewidth=4, color="tab:blue")

# Shade Student-t CI region (blue)
x_mid_t = x[(x >= x_t_lo) & (x <= x_t_hi)]
ax.fill_between(x_mid_t, 0, pdf_xbar(x_mid_t),
                color="tab:blue", alpha=0.25)

# Shade CLT (z) CI region (orange)
x_mid_clt = x[(x >= x_clt_lo) & (x <= x_clt_hi)]
ax.fill_between(x_mid_clt, 0, pdf_xbar(x_mid_clt),
                color="tab:orange", alpha=0.25)

# Student-t CI endpoints (blue)
ax.vlines([x_t_lo, x_t_hi],
          0,
          [pdf_xbar(x_t_lo), pdf_xbar(x_t_hi)],
          linewidth=3,
          color="tab:blue")
...
ax.vlines([x_clt_lo, x_clt_hi],
          0,
          [pdf_xbar(x_clt_lo), pdf_xbar(x_clt_hi)],
          linewidth=3,
          color="tab:orange")
...
ax.set_title(
    rf"Student's $t$ vs CLT CI for $\mu$ ($n={n}$, central {100*(1-alpha):.0f}%)",
    fontsize=24
)
ax.set_yticks([])
ax.tick_params(axis="x", labelsize=18)
...
ax.plot([], [], linewidth=4, color="tab:orange", label="CLT Normal ($z$)")
ax.legend(frameon=False, fontsize=16, loc="upper right")
...
3. $X_1,\dots,X_{100}$ and  $Y_1,\dots,Y_{100}$ are two IID samples of test scores from the same population of medical students. The $X_i$'s are the students' scores on a practice test, and the $Y_i$'s are the students' scores on the real test. Construct a 95% confidence interval for the mean difference in test scores between the practice and real tests.
