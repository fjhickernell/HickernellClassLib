...
- $\barX_n \exsim \Gam(n, n \lambda)$ if $X \sim \Exp(\lambda)$ where $\displaystyle \varrho_{\Gam(\alpha, \lambda)}(x) = \frac{\lambda^{\alpha}}{\Gamma(\alpha)}\, x^{\alpha-1} \exp(-\lambda x)
\quad x>0$ &nbsp; (Note: $\Gamma(n) = (n-1)!$ for integer $n$)
  - $\lambda n \barX_n \exsim \Gam(n, 1)$
  - $2 \lambda n \barX_n \exsim \chi^2_{2n}$, where $\displaystyle \varrho_{\chi^2_k}(x) = \frac{x^{k/2 - 1} \exp(-x/2)}{2^{k/2} \Gamma(k/2)} \quad x > 0$
...
- $\displaystyle \frac{\barX_n - \mu}{S/\sqrt{n}} \sim t_{n-1}$ if $X \sim \Norm(\mu,\sigma^2)$ where 
  ...
  - $t_k$ is the [Student's t distribution]{.alert} with $k$ degrees of freedom
  
    - $\varrho_{t_k}(x) = \frac{\Gamma\!\left(\frac{\nu+1}{2}\right)} {\sqrt{\nu\pi}\,\Gamma\!\left(\frac{\nu}{2}\right)} \left(1+\frac{x^2}{\nu}\right)^{-(\nu+1)/2}, \quad -\infty < x < \infty$
    - Symmetric about $0$
...
For the unbiased sample variance, $S^2$, for [$\Norm(\mu,\sigma^2)$]{.alert} based on IID data
